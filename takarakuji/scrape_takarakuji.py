...
def scrape_month(kind, ym):
    ...
    soup = get_soup(url)
    n_tables = len(soup.find_all('table')) if soup else -1
    print('    [debug] %s -> table数=%d' % (url, n_tables))
    # 月別URLでテーブルが取れない場合、月なしURL（今月分）を試す
    if not soup or not soup.find('table'):
        url = '%s/%s/' % (BASE, kind)
        soup = get_soup(url)
        n_tables2 = len(soup.find_all('table')) if soup else -1
        print('    [debug] fallback %s -> table数=%d' % (url, n_tables2))
    if not soup: return []
    results = []
    seen = set()
    for table in soup.find_all('table'):
        r = parse_one_round(table)
        print('    [debug] parse結果:', r['round'] if r else None)
